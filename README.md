...
タイトル、著者名、出版日、説明、画像URLが入力欄へ自動反映されることを確認してください。

---

## Scheduler・期限切れ通知

本プロジェクトでは、Laravel Schedulerを使用して期限切れ読書計画の通知処理を実行しています。

Schedulerには、毎日午前8時に期限切れ通知処理が登録されています。

登録されているスケジュールは、以下のコマンドで確認できます。
...
採点時や開発時にSchedulerの実行時刻を待たず確認できるよう、期限切れ通知を実行するArtisanコマンドを用意しています。
...
初回実行時は、期限切れの読書計画2件が処理され、ステータスが`expired`へ更新されるとともに通知が作成されます。

同じコマンドを再度実行すると、対象は0件となり、同じ読書計画に対して通知が重複作成されないことを確認できます。
...
通知一覧画面で、期限切れ通知が表示されること、および通知を既読にできることを確認してください。

本番環境では、CronなどからLaravel Schedulerを実行してください。
...
テストを実行する前に、Laravel Sailが起動していることを確認してください。
...
読書計画に関するテストのみ実行する場合は、以下のコマンドを実行します。
...
基本機能および応用機能について、要件で定められた以下のカバレッジを満たしていることを確認してください。
...
- お気に入り・レビューいいね
- ランキング
- 公開API・Sanctum認証・API認可
- ISBN検索
- 読書計画
- 通知・Scheduler
- マイ読書レポート
- Policy・モデルリレーション・カスケード削除
