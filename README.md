...
Docker Desktopを利用する場合は、Docker Desktopを起動してから作業してください。
...
```bash
git switch redo-advanced
```

ローカルに`redo-advanced`ブランチが存在しない場合は、以下を実行します。

```bash
git switch -c redo-advanced origin/redo-advanced
```
...
`.env`を開き、データベース接続情報が以下の内容になっていることを確認します。
...
`DB_HOST`には、`localhost`や`127.0.0.1`ではなく、Docker ComposeのMySQLサービス名である`mysql`を指定します。

---

## 3. PHP依存パッケージをインストール

クローン直後は`vendor`ディレクトリが存在しないため、通常のSailコマンドをまだ実行できません。

Dockerを利用してComposerパッケージをインストールします。
...
インストール後、`vendor`ディレクトリが作成されていることを確認します。
...
以下のコマンドで、Dockerコンテナをバックグラウンド起動します。
...
Laravel、MySQL、phpMyAdminなどのステータスが`Up`になっていれば起動成功です。
...
`.env`の`APP_KEY`に値が設定されたことを確認します。
...
マイグレーションを実行し、Seederの初期データを投入します。

```bash
./vendor/bin/sail artisan migrate:fresh --seed
```

既存のデータを削除せずにマイグレーションする場合は、以下を使用します。
...
`migrate:fresh`は、既存のテーブルとデータをすべて削除してから再構築します。

開発中のデータを残したい場合は、実行前に注意してください。

---

## 8. Vite開発サーバーを起動

別のターミナルを開き、プロジェクトディレクトリで以下を実行します。
...
作業終了時は、以下のコマンドでコンテナを停止できます。

```bash
./vendor/bin/sail stop
```

コンテナを削除して停止する場合は、以下を実行します。
...
毎回`./vendor/bin/sail`と入力せず、`sail`だけでコマンドを実行したい場合は、使用しているシェルに合わせてエイリアスを設定します。
...
M1・M2・M3などのApple Silicon搭載MacでMySQLイメージに関するエラーが発生する場合は、`compose.yaml`のMySQLサービスに以下を追加してください。
...
| No. | メソッド | URL | 内容 | 認証 |
| --- | --- | --- | --- | --- |
| AP01 | GET | `/api/v1/books` | 書籍一覧取得 | 不要 |
| AP02 | GET | `/api/v1/books/{book}` | 書籍詳細取得 | 不要 |
| AP03 | POST | `/api/v1/books` | 書籍登録 | Sanctum |
| AP04 | PUT | `/api/v1/books/{book}` | 書籍更新 | Sanctum・所有者のみ |
| AP05 | DELETE | `/api/v1/books/{book}` | 書籍削除 | Sanctum・所有者のみ |
| AP06 | POST | `/api/v1/login` | ログイン・トークン発行 | 不要 |
...
Laravel Pintを使用してコードを整形します。

```bash
./vendor/bin/sail pint
```

ファイルを変更せず、整形が必要かだけ確認する場合は以下を実行します。
...
Feature TestとUnit Testを実装し、以下の結果を達成しました。
