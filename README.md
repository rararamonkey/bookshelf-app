...
アプリでISBN検索機能を利用するには、Google Books APIキーを`.env`ファイルに設定してください。
...
環境変数を追加または変更した後は、設定キャッシュをクリアしてください。

```bash
./vendor/bin/sail artisan config:clear
```

APIキーが設定されていない場合、ISBN検索機能は正常に動作しません。。
...
既存のデータベースを削除して、最初から再構築する場合は以下を実行します。
